...
func init() {
	var err error
	snowflakeNode, err = snowflake.NewNode(int64(getEnvInt("ISUCON_SNOWFLAKE_NODE_ID", 1)))
	if err != nil {
		panic(err)
	}
...
func dispenseID(ctx context.Context) (string, error) {
	return fmt.Sprintf("%x", snowflakeNode.Generate().Int64()), nil
	//var id int64
	//var lastErr error
	//for i := 0; i < 100; i++ {
	//	var ret sql.Result
...
func playersListHandler(c echo.Context) error {
	...
	if err := tenantDB.SelectContext(
		ctx,
		&pls,
		"SELECT id, display_name, is_disqualified FROM player ORDER BY created_at DESC",
	); err != nil {
...
func playersAddHandler(c echo.Context) error {
	...
	if _, err := tenantDB.NamedExecContext(
		...
		return fmt.Errorf(
			"error Insert players at tenantDB: %w",
			err,
		)
	}
